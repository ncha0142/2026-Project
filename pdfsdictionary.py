...
def main():
    if __name__ == '__main__':
        if len(sys.argv) != 4: #This needs to be changed to 3 soon
            print('Usage: python BMedScDOC1_v6.py [dictionary.txt] [corpus_path] [csv.path]')
            sys.exit(1)
        ...
        df = tdm_make(docs) 
        print("tdm shape: ", df.shape)
    
        #convert pandas dataframe to csv
        location = input("Please enter the Path to directory where outputs should be saved. \n ie: /Users/manojarachige/Downloads/ \n Path: ")
        df.to_csv(os.path.join(location, "TDM.csv"), index=False)
        print("Saved TDM.csv to:", os.path.join(location, "TDM.csv"))
...
def remove_ref(text):
    f = open(text, "r", encoding="utf-8", errors="replace")
    openfile = f.read()
    ...
    f.close()
    n = open(text, "w", encoding="utf-8", errors="replace")
    n.write(newfile)
    n.close()
...
def pdftotxt(text):
    with open("{}.txt".format((text)[:-4]), "w", encoding="utf-8", errors="replace") as output:
...
def preprocess(string, csv_path):
    ...
    [word for word in lemmatised.split() if word.lower() not in STOP_WORDS]
)
...
def maketxt(text, csv_path):
    string = ''
    with open(text, "r", encoding="utf-8", errors="replace") as file:
        string = file.read()
